...
import { createViewer, type Viewer } from "@layerdraw/viewer";
import type { DesktopFeatureAvailability, DesktopLifecycleSnapshot, DesktopProjectContext, DesktopProjectPublicationDTO } from "./contracts.js";
...
function runtimeAdapter(client: WailsDesktopClient, host: DesktopProjectHostBinding): BrowserRuntimeClient {
  const adapter: BrowserRuntimeClient = {
    ...
    async materializeView(_input: MaterializeViewInput, _session, { signal }): Promise<ViewData> {
      if (signal.aborted) throw new DOMException("Aborted", "AbortError");
      throw new Error("No materializable Desktop view was advertised");
    },
...
export async function createDesktopWailsProjectOwner(host: DesktopProjectHostBinding, bindings: WailsGeneratedBindings): Promise<DesktopProjectOwnerBinding> {
  ...
  const publish = async (dto: DesktopProjectPublicationDTO): Promise<DesktopLifecycleSnapshot> => {
    ...
    if (project?.project_id !== dto.project.project_id || project.session_generation !== dto.project.session_generation) {
      ...
      project = Object.freeze({
        ...
        editor, editor_session: editorSession, views: [], access: { status: "allowed" as const, label: "Local owner" },
    ...
    return Object.freeze({ sequence: ++sequence, phase: dto.phase, capabilities: featureCapabilities(client.engine.getCapabilities()), project });
  };
  ...
  return Object.freeze({ ProjectPublication: refresh, SelectProjectView: success, ShowProjectRecoveryOptions: success, DisconnectProjectExternal: success, CreateProjectViewer: () => viewer });
}
