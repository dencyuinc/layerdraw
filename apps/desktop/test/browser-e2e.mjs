...
try {
  ...
  // Settings window: opened by the native menu event, hosts persisted general
  // settings and the MCP enable switch (moved out of the workspace inspector).
  ...
  assert.deepEqual(await page.evaluate(() => window.desktopWorkflow.calls.filter((call) => call[0] === "settings-load").length > 0), true);
