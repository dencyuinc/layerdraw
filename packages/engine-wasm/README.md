...
`transport.closed` settles only after the Worker and every in-flight exchange are terminal. Callers use it to join explicit termination and unexpected Worker failure without retaining stale endpoint resources.
...
Workbench calls use the same Engine Protocol operations as native and stdio
delivery. Browser-specific code owns only Worker lifecycle, byte transfer,
hard-cancel replacement, and artifact verification; it must not reinterpret
Working Document handles, cursors, previews, AuthoringImpact, or source edits.

The initial tested shell requires a dedicated module Worker, WebAssembly, transferable fixed `ArrayBuffer`, structured clone, `crypto.getRandomValues`, `performance.now`, `TextEncoder`/`TextDecoder`, `Blob` object URLs, and `fetch` or an adapter-provided verified-byte loader. Every loaded artifact is copied into a Worker-owned byte snapshot before validation; the verified `wasm_exec.js` snapshot is executed through a short-lived object URL that is revoked after module evaluation, and the verified WASM snapshot is instantiated directly. Playwright is pinned to 1.61.1 with Chromium 149.0.7827.55 (revision 1228), Firefox 151.0 (revision 1532), and WebKit 26.5 (revision 2311); the checked-in test metadata is the exact matrix input. This does not claim Node as a production transport, SharedWorker, Service Worker, `SharedArrayBuffer`, WASM threads, mobile browsers, embedded WebViews, or a portable browser process-memory ceiling.
