...
func validateLocallyDerivableViewDependencies(object map[string]any) bool {
	...
	sets := newViewDependencySets(); collectViewDependencyAddresses(source,&sets); collectViewDependencyAddresses(shape,&sets); for address := range overrides { sets.add(address) }
...
func validateViewRecipeConsistency(path string, object map[string]any) error {
	...
	if shapeKind == "table" {
		...
		dependencies, _ := object["dependencies"].(map[string]any); stateReads, _ := dependencies["state_reads"].([]any); for _, read := range directReads { if !containsStateRead(stateReads,read) { return fmt.Errorf("%s dependencies omit a direct Table state read", path) } }; if _, hasSourceQuery := source["query_address"].(string); !hasSourceQuery && len(stateReads) != len(directReads) { return fmt.Errorf("%s dependencies contain non-derivable state reads", path) }
