...
`initialize` 成功値は `{ ok: true, endpoint_generation, engine_release, source_revision, protocol_schema_digest, go_version, module_build_info, sbom_authority_digest, transport_limits }` である。release/source、生成 protocol digest、実行中 Go binary の build info、および actual build graph＋license policy から link-inject した SBOM authority digest は linked Go authority から返される。Go build は同じ graph＋policy から canonical `engine-wasm.authority.json` を生成し、その完全なbytesのdigestをmanifestとbinaryへlinkする。TypeScript はそのbytes/digestを検証し、manifest runtime/legal tuple、CycloneDX components/licenses/dependency projection、bridge build infoと機械的にexact compareするだけであり、schema digest literal、module/legal value、Go projection algorithmを複製してはならない。相違が一つでもあれば `Ready` を公開してはならない。
...
artifact は Go `1.26.6`、`GOOS=js`、`GOARCH=wasm`、`CGO_ENABLED=0`、`GOWORK=off`、空 `GOEXPERIMENT`、`GOENV=off`、`GOFLAGS=-mod=readonly` で build する。`-trimpath -buildvcs=false` と `-ldflags=-buildid= -s -w` を使用し、release version、40 桁 lowercase source revision、actual js/wasm module graph と reviewed license policy の canonical SBOM authority digest だけを link する。時刻、hostname、absolute path、random identity、endpoint identity、artifact manifest digest を link してはならない。

`wasm_exec.js` は同じ Go `1.26.6` の `GOROOT/lib/wasm/wasm_exec.js` を byte-for-byte copy する。期待 SHA-256 は `0c949f4996f9a89698e4b5c586de32249c3b69b7baadb64d220073cc04acba14` である。digest が変わった場合は build を失敗させ、Go/tool support pair を同時に明示 review しなければならない。minify、transpile、patch、download substitute を禁止する。

artifact-local manifest は canonical JSON で、build flags、source/release、生成 Engine schema digest、固定 transport/compiler limits、Go生成`engine-wasm.authority.json`のpath/digest、`layerdraw-engine.wasm`、`wasm_exec.js`、legal files、artifact SBOM の size/media type/SHA-256 を含む。manifest 自身の digest field は持たない。canonical authority artifact はactual js/wasm build graphとrepository license policyから、manifest runtime/legal tuple、bridge build info、closed SBOM components/licenses/dependenciesを一度だけ生成する。CycloneDX 1.6 SBOM は runtime Go module と co-distributed Go WASM runtime support を区別し、root/component reference と dependency graph を closed にし、`LICENSE`、`NOTICE`、`LICENSING.md`、`THIRD_PARTY_NOTICES.txt` を同梱する。production loader は artifact 外の npm `package.json` version、manifest、authority artifact、SBOM、linked Go initialization を `Ready` 前に一致させ、artifact verifier は package version を明示的な `-version` authority として要求する。
