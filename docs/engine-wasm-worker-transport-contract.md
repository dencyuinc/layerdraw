...
```text
initialize(endpointGeneration, endpointInstanceID, releaseManifestDigest)
...
handshake を再実行する時は既存 negotiated context を先に破棄する。成功時だけ新 context を保存する。rejected、failed、cancelled handshake 後は unnegotiated のままであり、以前の context で compile してはならない。
...
| `engine.worker.transfer_failed` | `transfer` | false（copy の一時失敗だけ true 可） |
