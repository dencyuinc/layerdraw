...
artifact-local manifest は canonical JSON で、build flags、source/release、生成 Engine schema digest、固定 transport/compiler limits、actual js/wasm build graph と repository license policy から生成した closed SBOM component authority、`layerdraw-engine.wasm`、`wasm_exec.js`、legal files、artifact SBOM の size/media type/SHA-256 を含む。manifest 自身の digest field は持たない。CycloneDX 1.6 SBOM は runtime Go module と co-distributed Go WASM runtime support を区別し、root/component reference と dependency graph を closed にし、`LICENSE`、`NOTICE`、`LICENSING.md`、`THIRD_PARTY_NOTICES.txt` を同梱する。production loader は artifact 外の npm `package.json` version、manifest、SBOM、linked Go initialization を `Ready` 前に一致させ、artifact verifier は package version を明示的な `-version` authority として要求する。
