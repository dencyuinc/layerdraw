...
Go EngineとTS RenderはAWSやERを直接知らない。
Go EngineはPackが宣言したRelationTypeをViewDataへ変換し、Export時はExportPlanを生成する。TS Render / versioned serializerはViewData、RenderData、ExportPlanとrender hintsからvisual output / ExportArtifactを生成する共通機構だけを持つ。
...
PackはGo Engineが知っているprojection primitiveとTS Renderが知っているrender primitiveを宣言するだけで、それぞれの規範componentが宣言を解釈する。
...
完全に新しい意味表現はRelationTypeへ埋め込まず、Go EngineのViewShape / ProjectionModeとTS Renderのversioned primitiveとして追加する。
...
この設計なら、新しいRelationTypeとprojection / render hintsをPackで追加するだけで、Go Engine / TS Renderが知っているprimitiveの範囲では新しい合成表示を使える。
...
Go Engine validation must cover:
...
- Projection rule uses a mode unsupported by Go Engine
