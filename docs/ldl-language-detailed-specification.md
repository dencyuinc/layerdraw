...
`resolved_requirements`はhostがregistryとprofile specificationを検証して解決したclosed inputであり、Engineがcaller supplied listをregistry-verifiedとみなすものではない。hostはexporter/serializer両profile参照をrecipeの`exporter_profile`とidentity、format、registry schema/digest、specification digestまで完全一致させ、必要asset/font digestのexact setをUnicode scalar順の重複なし配列として渡す。Engineはこの完全一致とclosed schemaを検証し、そのexact setを`profile_requirements_hash`、Planのrequired digest集合、Source Manifestへ束縛する。Engine境界でregistry clientやserializerを実行してはならない。
...
ExportInvocationInput {
  ...
  recipe: ExportRecipe
  resolved_requirements: ResolvedExportProfileRequirements
...
ResolvedExportProfileRequirements {
  ...
  required_asset_digests: sha256[]
  required_font_digests: sha256[]
}
...
ExportPlan {
  ...
  profile_requirements_hash: sha256
...
ExportPlanUnit {
  ...
  artifact_role: string
  role: string
  kind: enum(section,sheet,page,slide)
  order: uint64
  viewdata_keys: string[]
}
```

`artifacts`は`(primary desc,role,logical_path)`、`units`はorder、`representations`は`viewdata-root`を先頭、続いてViewData item key順にsortする。unitはartifact内のsection/sheet/page/slideであり、artifactそのものと同一視しない。非omitted representationの`artifact_role`は既存artifactを、`unit_id`は同じartifactに属する既存unitを必ず指し、そのViewData keyはunitの`viewdata_keys`へ含める。`lossless`はcomplete ViewDataを`embedded`するreserved `viewdata_key:"viewdata-root"` representationを必須とし、そのsourceはViewDataBase.sourceとする。他のrepresentationはpresentationである。`traceable_summary`は全primary semantic itemを`rendered`または`tabular`にし、support itemを省略する場合も`omitted`と理由とSourceRefsを残す。`visual_only`は表示対象itemだけを対応付け、`lossy`は完全な対応を要求しない。

ExportPlan生成前に、再帰的なnested collectionとcollection間を含む全ViewData item keyがglobalに一意であることを検証する。Treeの`roots`と全階層の`children`はrole `occurrences`、DiffChangeの`fields`はrole `field_diffs`へ正規化する。itemが直接`source`を持たない場合はnearest enclosing itemのSourceRefsを継承し、該当する親がなければViewDataBase.sourceを使う。この規則によりTableColumnはViewDataBase.sourceを、FieldDiffはenclosing DiffChange.sourceを継承する。keyを持つitemをsource欠落だけで黙ってrepresentation集合から除外してはならない。
...
`view_data_hash`、`recipe_hash`、`state_summary_hash`、`profile_ref_hash`、`profile_requirements_hash`、`invocation_hash`は9節の各export domainで計算する。Exportの`view_data_hash`は意味上の表示同一性を測る`viewdata` domainではなく、revision、shape、SourceRefs、StateRefs、state policy、StateInputRefを含む`export-viewdata` domainを必ず使う。Planの`serializer_options`はcanonical ExportRecipeのcomplete closed `options`と一致し、serializerはExportRecipeを再読込または再解釈しない。Plan/Manifestのexporter/serializer profile identity、`state_policy`と`state_input`は完全一致し、許可する組合せを`none -> none`、`required -> snapshot`、`optional -> none|snapshot`に固定する。snapshot利用時はhash/version/captured_at/definition hashを保持する。state summaryがない場合だけ`state_summary_hash`をabsentとし、`export-invocation` payloadではJSON `null`を使う。PlanとManifestの6hash値、profile identity、state policy、state inputは一致しなければならず、recipe revisionから`recipe_address`を解決して再計算した値と異なるartifactは拒否する。これによりrecipe optionsとhost-resolved requirementsを含むinvocation全体を、recipe addressだけに依存せず束縛する。
...
domainは`resolved`、`definition`、`graph`、`state-query-snapshot`、`query`、`view`、`viewdata`、`export-viewdata`、`export-recipe`、`export-profile-ref`、`export-profile-requirements`、`export-state-summary`、`export-invocation`、`subject`、`subtree`、`child-set`、`operation-batch`、`viewdata-item`とし、異なるdomainのハッシュを交換可能として扱わない。
...
- export-profile-requirements payloadはcomplete ResolvedExportProfileRequirements。digest配列はUnicode scalar順かつ重複なし
- export-state-summary payloadはcomplete ExternalStateSummary
- export-invocation payloadは`{view_data_hash,recipe_hash,state_summary_hash,profile_ref_hash,profile_requirements_hash}`。state summary absent時の値はJSON `null`
