...
View = Common & {
  ...
  reserved_table_column_ids: set<string>
  reserved_export_ids: set<string>
}
...
LDL source syntax generationとgenerated スキーマ versionは分離する。Source fileはversion headerを持たない。Language 1はnormalized スキーマ `1`、resolved スキーマ `1`、StateQuerySnapshotスキーマ`1`、セマンティック index スキーマ `1`、診断 protocol `1`、operation protocol `1`から開始する。

必須フィールド、enum意味、normalized 既定値、ordering、ハッシュ input、reference表現の追加・変更はgenerated-schema major versionを上げる。セマンティックでないoptional ソース/index フィールドはminor metadata versionで追加できるがセマンティック ハッシュを変えてはならない。readerは未知 higher majorを拒否する。older majorからのmigrationは明示的・決定的に新artifactを生成し、未知フィールドを推測または黙って破棄しない。
