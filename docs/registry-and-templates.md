...
Pack manifestは、Go Engineが解釈できるprojection primitiveとTS Renderが解釈できるrender primitiveだけを宣言できる。
...
Registry clientはartifact bytesを取得する。Go Registry componentはsource identity、publisher / signature policy、dependency graph、install transactionを所有し、Go Engine package componentはZIP safety、artifact manifest、LDL、resolved tree、canonical digestを検証・生成する。TS Library UI、provider shell、MCP adapterはこの検証を再実装しない。
