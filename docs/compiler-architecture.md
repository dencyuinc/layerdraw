...
Project Search用のSearchDocumentもNormalizedDocumentとSourceMapからGo Engineが決定論的に生成する。Embedding vector、Ladybug index内部ID、Actor固有Access結果はCompiler出力に含めない。
...
SearchRequest + fixed revision + Search Index identity
  ...
  -> typed raw hits
  -> Go Engine ranking / RRF / source binding
  -> canonical SearchResult
```

Go Engineはcorpus文字列化、filter、Hybrid fusion、StableAddress binding、orderingを所有する。Runtimeはrevision、Access、index lifecycle、Embedding Providerを所有する。execution adapterはFTS / Vectorを物理実行するだけである。
...
canonical QueryResult or explicit revision-bound subgraph
  ...
  -> typed metric rows
  -> Go Engine StableAddress binding / ordering / normalization
...
`.layerdraw` / `.ldpack` container writer は Go Engine の package component を使う。plain visual export と portable container export を同じ責務にしない。
