...
- UTF-8 bytes として Go Engine へ渡す
- Go Engine が返した diagnostics、source range、canonical source を表示する
...
Framework を変更しても、Go Engine、protocol、ViewData、semantic operation の意味は変わらない。
...
Organization / Workspace所属はLDL definition、StableAddress、portable `.layerdraw`へ含めない。Go Runtime library単体はOrganizationを知らず、Server ApplicationがOrganization-scoped Document ID、Actor / Access context、storage bindingを解決してRuntimeへ渡す。
...
protocol wire shapeはmonorepoの`schemas/`に置く言語非依存schemaを正本とする。Go Engineはsemantic validation behaviorを所有するが、Go structをwire schemaの正本にしない。JSON Schema、Protocol Buffers、または同等のcode generation可能なschemaを使い、次を生成する。
...
診断 code、StableAddress、source span、hash の算出は Go Engine だけが行う。
...
Server、Desktop、native sidecar では Go Engine が Go binding を通じて LadybugDB を実行する。
...
browser では LadybugDB WASM を TS adapter から呼べる。ただし TS adapter は Go Engine が生成した parameterized Query / Search / Analysis ExecutionPlanを機械的に実行し、typed raw rows を Go Engine WASM へ返すだけである。
...
WASM、sidecar、serverは同じGo Engine sourceとconformance suiteからbuildする。
...
| `@layerdraw/query-adapter-ladybug-wasm` | Go EngineのQuery / Search / Analysis ExecutionPlanとSearchIndexPlanをLadybug WASMで実行しtyped raw rowsだけを返すbrowser adapter |
...
| SDK / Browser Editor variant | Go Engine WASM Worker | TS Engine client + React optional | browser / injected host adapter |
...
SaaS / Self-host の browser は通常 server を権威 host とする。低遅延 preview のため Go Engine WASM を併用してよいが、commit は server が再検証し、client preview を信用しない。
...
Cloudflare Workers のように Go native binary を実行できない環境は、LayerDraw Server 互換 API を独自の TS semantic implementation で作らない。Go Engine を実行できる service / WASM host へ委譲する。deployment convenience のために意味論を二重化しない。
...
CRDT / OT / command log は transport と Working Document coordination の選択肢であり、Committed Revision の意味論を変更しない。すべての commit は Go Engine の validation と canonicalization を通る。
...
Go Engine は React、Echo、Wails、VSCode、Node package に依存しない。TS presentation package は `@layerdraw/protocol` と client facade を通じて Engine を利用する。private cloudはpublic Go `internal/`をimportせず、署名済みrelease artifactとversioned public protocolを使う。
