...
Dependency licenseの機械可読な正本は[`tools/license-policy.json`](../tools/license-policy.json)とする。`make license-check`はsource SPDX、npm manifest、Go module review、pnpm license metadataを検証し、許可表にないlicense、本文digest変更、未審査module、不要になったoverrideを拒否する。同時に全runtime / development依存と入力policy / lockfile digestを`reports/dependency-licenses.json`へ出力する。Go moduleのreviewはpathとversionへ束縛し、dependency updateでlicense本文またはversionが変わった場合は同じPull Requestで再確認する。

`make package`はbuild済みbinaryのGo build infoを入力にしてruntime dependency closureを確定し、`dist/`へ正準License、Notice、licensing matrix、third-party notice、CycloneDX 1.6 SBOMを生成する。CIで使用したtool dependencyやrepository全体のmodule graphを配布依存へ混入させない。npm tarball、container、WASM、VSIX、Desktop installerを追加するときも同じ原則でartifact固有のclosureを入力にし、package testで同梱とparse可能性を検証する。
