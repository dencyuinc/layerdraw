...
| Browser Editor | Viewer一式、`engine-client/wasm`、`engine-wasm`、`composer`、`client-sdk`、local Search / Query / Analysis時は`query-adapter-ladybug-wasm`、local semantic Search時は`embedding-provider-wasm`、optional `react` | LayerDraw Engine WASM。永続Runtimeはhost injectionまたはremote host |
...
Browser EditorのLayerDraw Engine WASMはAuthoringImpactを生成できるがActor / Policy decisionを所有しない。fixed-schemaをauthoritativeに強制するSDK構成は`layerdraw-host`、`layerdraw-server`、または同契約のtrusted Runtime endpointを必須とする。local-only callbackでのcontrol非表示をsecurity boundaryとして扱わない。
...
Engine / Runtimeは接続先hostが所有する。MCP Apps clientへLayerDraw Engineを暗黙bundleしない。
...
Legacy packageをcompatibility layerとして残す場合も、LayerDraw Engineを呼ぶfacadeに限定し、TS semantic implementationを保持してはならない。
