...
wire互換性: Access Protocol v1の`EvaluateAuthoringInput.base_revision_digest`は必須fieldである。wire clientは更新済みschemaからbindingを再生成し、Runtimeが返したcurrent revisionを基に評価を要求する。Go Runtimeのin-process `Authorize`は移行期間中、空の値だけをtrusted current revisionから補完するが、異なる非空値を受理しない。
