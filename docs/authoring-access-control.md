...
| --- | --- | --- |
| Definition差分 | `semantic/AuthoringImpact` | Go Engine |
| Capability、Host operation、Policy、Grant、Decision | `access-protocol` | Go Access。Policy persistence / bindingはServer Application |
| preview / commit precondition | `runtime-protocol/AuthoringProof` | Go Runtime |

schema directoryは実装責務を意味しない。`access-protocol`は言語中立wire正本であり、TS policy evaluatorや独立serviceを要求しない。
