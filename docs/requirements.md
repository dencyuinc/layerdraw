...
LayerDraw は、業務・データ・アプリケーション・インフラ・ネットワークを 1 つの意味付きグラフとして管理し、必要な切り口で 2D / 3D の構成図へ投影するためのエディタ / レンダラである。
...
- Layer を奥行きまたは帯として表現する 3D 図を生成できる
...
- browserではTS Renderがlayout / renderingを行い、LDL解釈が必要なclient提供形態はLayerDraw Engine WASM Workerを使える構造にする
...
Authoring AccessはDefinition変更後の意味をLayerDraw EngineがAuthoringImpactへ分類し、versioned owner protocolがLDL外writeをHostOperationImpactへ固定する。LayerDraw AccessがActor / role / agent delegation / policyと両impactを照合し、document / asset / package適用はLayerDraw Runtime、Host metadataはServer Applicationまたはlocal Host Applicationがpublication直前に原子的に強制する。
...
2D 無限キャンバスとThree.jsによる3D可視化を標準の構造確認面として扱う。
...
- LayerDraw Engine内部はlossless CST、typed AST、JSON互換NormalizedDocumentを扱う
...
- そのため、LayerDraw Engineがsourceをtyped AST、NormalizedDocument、SemanticIndexへcompileする
...
- LDL意味論はLayerDraw Engine（Go実装）だけが実装し、native library、WASM、stdio sidecar、server-linked artifactとして提供する
...
- browser editorはLayerDraw Engine WASM Workerを使い、server-backed WebはGo serverを権威hostにする
- RendererはTS Render packageの決定的visual rendererとし、ViewDataからSVG / PNG / bundle exportを提供する
- 自動レイアウトはTS Renderのlayered-2d / stacked-3d layoutとし、ViewDataの意味を変更しない
- Cloudflare等のdeploymentはLayerDraw Engineを実行できるservice / WASM hostへ接続し、TSで意味論を再実装しない
...
- LayerDraw EngineがDSL source treeからNormalizedDocumentを一方向生成して検証する。generated JSONからsourceを復元・修復せず、外部JSON importが必要な場合は明示的なimport / migrationをGo Workbench経由で新しいLDL sourceとして生成する
...
- 表示モードは layered-2d と stacked-3d を持つ
...
ViewDataからRenderDataを生成し、2D / 3Dの図として描画する処理。

## 11. 3D 表現

LayerDraw における 3D は、見た目を装飾するための疑似 3D ではない。多層構造を奥行き方向として扱うための表現である。
