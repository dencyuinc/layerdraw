...
Target packageとして定義していないlegacy package名はpublish対象にしない。compatibility facadeを別途残す決定をした場合も、Go Engineを呼ぶ薄いartifactに限定し、TS semantic implementationを保持してはならない。
