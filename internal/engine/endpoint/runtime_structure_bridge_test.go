...
`
	...
	if len(structure.EntityTypes) != 1 || len(structure.EntityTypes[0].Columns) != 2 || structure.EntityTypes[0].Columns[0].ValueType != "enum" {
	...
	if api == nil || len(api.Rows) != 1 || len(api.Rows[0].Values) != 2 {
	...
	if values["enum"] != "prod" || values["integer"] != "75" {
