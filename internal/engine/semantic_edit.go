...
func validateSemanticPreconditions(snapshot Snapshot, pre SemanticEditPreconditions, batch SemanticOperationBatch) []SemanticConflict {
	...
	for _, file := range snapshot.SourceMap.Files {
		if file.Origin.Kind == "project" {
			files[file.ModulePath] = file.Digest
		}
	}
	...
	for _, expected := range pre.ExpectedSourceDigests {
		if files[expected.ModulePath] != expected.Digest {
			conflicts = append(conflicts, SemanticConflict{Kind: ConflictSubjectChanged, Path: []string{expected.ModulePath}})
		}
	}
	if len(conflicts) != 0 {
		sortSemanticConflicts(conflicts)
	...
	for _, operation := range batch.Operations {
		...
		case OperationDeleteSubject, OperationRenameSubject:
			...
			if subject, ok := subjects[operation.TargetAddress]; ok {
				...
				if subject.SubtreeHash != nil {
					if _, present := providedSubtrees[operation.TargetAddress]; !present {
						conflicts = append(conflicts, SemanticConflict{Kind: ConflictSubtreeChanged, TargetAddress: operation.TargetAddress})
					}
				}
		...
		case OperationMigrateProjectIdentity:
			if _, ok := providedSubtrees[operation.ProjectAddress]; !ok {
				conflicts = append(conflicts, SemanticConflict{Kind: ConflictSubtreeChanged, TargetAddress: operation.ProjectAddress})
			}
		case OperationUpdateRelationEndpoint:
...
func sortSemanticConflicts(conflicts []SemanticConflict) {
	...
	sort.SliceStable(conflicts, func(i, j int) bool {
		...
		if a.ChildKind != b.ChildKind {
			return a.ChildKind < b.ChildKind
		}
